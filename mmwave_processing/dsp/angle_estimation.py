...
def aoa_capon(x, steering_vector, magnitude=False, diag_load=0):
    ...
    first = Rxx_inv @ steering_vector.T                                         # (Ant index, Angle index)
    den = np.reciprocal(np.einsum('ij,ij->i', steering_vector.conj(), first.T)) # (Angle index)
    weights = np.matmul(first, den)                                             # (Ant index)
