...
class Console(Cmd):
    ...
    def __processing_thread(self):
        ...
        Range_Spec = np.zeros((self.NUM_FRAMES, self.configs.num_chirps_per_frame, self.configs.num_samples_per_chirp//2), dtype=np.float64)    # (60, 32, 32)
        uDoppler = np.zeros((self.NUM_FRAMES, self.configs.num_samples_per_chirp//2, self.configs.num_chirps_per_frame), dtype=np.float64)      # (60, 32, 32)
        Angle_Spec = np.zeros((self.NUM_FRAMES, self.ANGLE_BINS+1, self.configs.num_samples_per_chirp//2), dtype=np.float64)                    # (60, 32, 32)
        ...
        while True:
            ...
            #------------------------------#
            # 3. Range Profile Spectrogram #
            #------------------------------#
            
            Range_Spec[:-1,:,:] = Range_Spec[1:,:,:]
            Range_Spec[-1,:,:] = np.sum(np.abs(radar_cube_clutter_removed)**2, axis=1)  # (num_frames, num_chirps, range bins) = (60, 32, 32)
            
            range_spec_img = np.sum(Range_Spec, axis=1).T                               # (range_bins, num_frames) = (32, 60)
            ...
            range_doppler, aoa_input, _ = dsp.doppler_processing(radar_cube_clutter_removed, num_tx_antennas=self.configs.NUM_TX, 
                                                                 n_fft=self.configs.DOPPLER_FFT, window_type_2d=Window.HANNING, pwr=True)
            ...
            uDoppler[:-1,:,:] = uDoppler[1:,:,:]            # (NUM_FRAMES, range bins, doppler bins) = (60, 32, 32)
            uDoppler[-1,:,:] = RD_Map
            
            uDoppler_img = np.sum(uDoppler, axis=1).T       # (doppler bins, num_frames) = (32, 60)
            ...
            Angle_Spec[:-1,:,:] = Angle_Spec[1:,:,:]        # (NUM_FRAMES, num_beams, range bins) = (60, 32, 32)
            Angle_Spec[-1,:-1,:] = range_azimuth**2
            
            angle_spec_img = np.sum(Angle_Spec, axis=2).T   # (num_beams, num_frames) = (32, 60)
